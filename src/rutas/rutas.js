...
rutas.get('/Categoria1', async (req, res) => {
    let productos = await pool.query('SELECT DISTINCT producto.id_Producto AS ID, producto.Nombre AS Nombre, producto.Ruta_Imagen AS Ruta, categoria.Nombre as categoria from producto inner join categoria on categoria.id_categoria=producto.Id_idcategoria ');
...
function CrearPDF(req, res) {
    ...
    var line = "___________________________________________________________________________________________________";
    ...
    doc.text("|                                DESCRIPCION", 230, 185, { width: 300 });
    ...
    doc.text("NUESTROS PRODUCTOS SIEMPRE SON DE MAXIMA CALIDAD", 40, 273, { width: 300 });
    doc.text("VALOR TOTAL  :", 425, 260, { width: 150 });
    doc.text(valortotal, 500, 260, { width: 150 })


    doc.fontSize(8).text("Sistema desarrollado por Karen Daniela", 40, 330, { width: 300 });
    doc.end();
